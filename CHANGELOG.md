...
- **MG_CACHE2** — фикс reconnect-гонки: (1) ретрай `OfflineToggleQueue.flush()` по таймеру (3 с), пока очередь не пуста и онлайн — событие `connectivity online` опережало реальную готовность сети, первый flush падал → «зависшая синхронизация»; (2) `pendingForList()` + `_applyPending()` в `_onDetail` — переналожение незасинканных тогглов поверх ответа сервера, устраняет откат вычеркнутого товара после возврата сети. Правки `offline_toggle_queue.dart`, `shopping_bloc.dart`.
