...
- ✅ Веб-деплой подтверждён пользователем (задача 1 сессии).
- ✅ **Mobile free-UX**: Холодильник и Дневник скрыты из нижней навигации для free-юзеров;
  прямой переход на `/fridge`/`/diary` → редирект на `/paywall`; баннер «Подписка истекла»
  скрыт для free (он актуален только для истёкших premium). APK пересобирается через CI.
- ⏳ Задача 3: дружелюбный текст вместо сырого «Лимит участников для тарифа „Бесплатный”
  исчерпан (1)» на странице Семьи (для free) — **не сделано, следующая сессия**.
- ⏳ Mobile: прогнать реальную проверку квоты в APK против сервера (скачать свежий APK
  из Actions после того как CI позеленеет).

### ⚠️ Ещё один грабель — запомнить навсегда (сессия jolly-heisenberg)

11. **Лintер/хук откатывает файлы на feature-ветке.** При коммите на `claude/*`-ветке
    хук может откатить изменённые файлы к HEAD (`main`). Симптом: `git status` чистый,
    но файлы — оригинальные. Лечение: **сразу работать в `main`** — не коммитить на
    `claude/*`, а напрямую коммитить и пушить в `main`.

12. **`PremiumGateState.hasEverPremium`** — новый флаг (коммит `a92927c`). Без него
    `reportReadSuccess()` в `MenuBloc` продвигал free-юзера из `lockedForRead` →
    `lockedForWrite`, и на экране «Меню» показывались все 6 табов + баннер «Подписка истекла».
    Теперь продвижение возможно только при `hasEverPremium=true`.

## 6. Ветки/коммиты
- Всё в `main` (последний: `5b3a5ba`). Линия freemium → mobile-free-UX:
  `b62bf30` (backend) → `1f889a6` (web/mobile UI) → `39eee44` (setup script) →
  `0733340` (web-гейт) → `583a120`/`74ee0df` (deploy-скрипты) →
  `39cf4af` (mobile: скрыть табы) → `a92927c` (fix: hasEverPremium) →
  `5b3a5ba` (fix tests).
