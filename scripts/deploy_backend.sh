...
# Код берём ТОЛЬКО из curl. Раньше стояло `$(curl ... || echo "000")`: при
# недоступном адресе curl сам печатает 000 И возвращает ненулевой код, echo
# дописывал ещё три нуля — получалось «000000», которое не равно «000» и не
# начинается с «5», то есть проверка ниже пропускала недоступный backend как
# успех и печатала «ГОТОВО».
# 2 минуты, а не 40 секунд: после синхронизации кода меняются все файлы разом,
# и runserver перекомпилирует дерево целиком — на проде это заметно дольше,
# чем обычный рестарт. Ложная тревога здесь хуже ожидания: она заставляет
# думать про откат там, где надо было просто подождать.
code="000"
for _ in $(seq 1 "${HEALTH_RETRIES:-60}"); do
  sleep 2
  code=$(curl -s -o /dev/null -w '%{http_code}' "$HEALTH_URL" 2>/dev/null) || code="000"
...
if [ "$HEALTH_BAD" = "1" ]; then
  echo "!! Backend не отвечает (HTTP $code) после ожидания. Проверь логи:"
