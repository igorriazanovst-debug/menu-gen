...
### T-25 — приглашение в семью не работает ни у кого · severity: high · OPEN (chat-84)
...
Патч: свести восемь копий в один общий резолвер с явным, задокументированным
правилом и покрыть тестами. Сам по себе он маленький, но упирается в вопрос,
ответ на который даёт T-26: если членств два, какое считать рабочим. Пока T-26
не сделана, менять правило на «самое новое» нельзя — это молча переселило бы
...
### T-28 — дневник, вода и вес привязаны к членству, а не к человеку · severity: medium · OPEN (chat-84)
...
Патч: перевести три таблицы на пользователя (или, как минимум, вес и воду).
Единственный из четырёх пунктов, который трогает данные на живой базе, — делать
отдельным шагом с dry-run. По порядку разумно: T-25 → T-28 → T-26 → T-27, иначе
первое же переключение спрячет у человека его собственные замеры веса.
