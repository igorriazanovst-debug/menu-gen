...
### T-21 — s1: пять десертов в каждом меню при пуле 45 · severity: medium · OPEN (chat-83)
`mg_analyze_s1_repeats --runs 20 --days 7` после публикации say7: пять рецептов
встречаются в **20 прогонах из 20**, и все пять — десерты (3944, 4234, 3975,
4172, 4224). При этом пул десертов 45 при потребности 28, то есть формально
«🟢 ок».

Значит дело НЕ в размере пула: до слота доходит горстка. Первая гипотеза
(«нет `portion_g`, рецепт не проходит коридор калорий») **проверена и не
подтвердилась** — годных десертов 44 из 45.

Что известно про механику отбора (`_pick_for_role`, `apps/menu/generator.py`):

* десерт берётся только в обед и не чаще `DESSERT_MAX_PER_DAY` вместе с
  выпечкой — то есть за прогон это максимум ~7 попаданий;
* кандидат отсеивается, если `suitable_for` НЕ пуст и не содержит тип приёма
  (`lunch`). Это главный подозреваемый: если у большинства десертов
  `suitable_for` без `lunch`, останутся ровно те немногие, у кого он пуст или
  подходит;
* калорийное окно ±15/30/50 % считается от `per_meal_cal / len(roles)`, а в
  обеде пять ролей — на десерт приходится около пятой части бюджета. При 1376
  ккал у рецепта 3944 он не проходит ни одно окно, но в этом случае фильтр
  отдаёт всех кандидатов целиком, поэтому калории вряд ли и есть разделитель.

Проверять так (только чтение):

```
docker compose exec -T backend python manage.py shell -c "
from collections import Counter
from apps.recipes.models import Recipe
qs = list(Recipe.objects.filter(is_published=True, dish_type='dessert'))
print('десертов:', len(qs))
print('suitable_for пустой:', sum(1 for r in qs if not r.suitable_for))
print('содержит lunch:', sum(1 for r in qs if r.suitable_for and 'lunch' in r.suitable_for))
print(Counter(tuple(sorted(r.suitable_for or [])) for r in qs))
for r in Recipe.objects.filter(id__in=[3944, 4234, 3975, 4172, 4224]):
    print(r.id, r.suitable_for, r.kcal)
"
```

Важно для T-17: пока это не разобрано, добор новых рецептов может не помочь —
новые попадут в тот же отсев.
...
Остаток задачи — только перекусы (+26). Но прежде чем искать новые рецепты, см.
T-21: повторяемость теперь идёт от десертов, у которых пул формально достаточен,
и там дело не в размере пула.
