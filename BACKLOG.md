...
## Новые задачи (chat-86)

### T-36 — штуки и упаковки не сходятся с граммами · severity: medium · ✅ CLOSED (MG_UNITNORM, chat-86)

Холодильник хранит то, что человек принёс из магазина: яйца в штуках, творог и
сметану в упаковках. Рецепты и список покупок считают в граммах. Перевести одно
в другое нечем — веса штуки и объёма упаковки у продукта нет ни в модели
(`Product` знает только `default_unit`), ни где-либо в коде.

Следствие видно на проде: у блюда «Запеченный омлет с тестом фило» потребность
«Яйца куриные 50 г» не сошлась с позицией «яйца 30 шт», хотя продукт
определился верно (#41, синоним отработал). Дальше по тому же правилу
расходятся творог и сметана в `упаковка`.

Это **не дефект списания**, а общий предел: вычитание холодильника при сборке
списка покупок (`_subtract_fridge` в `apps/shopping/services.py`) считает теми
же `_fr_base`/`_fr_unit_factor`. То есть яйца из списка покупок уже сегодня не
вычитаются — молча. Списание (MG_WRITEOFF) это лишь показало.

**Починено.** Заведена таблица `ProductUnitWeight` — «одна единица U товара P
весит N граммов». Она не про штуки: той же строкой описываются `упаковка`,
`банка`, `пучок` и `л` (для жидкого это заодно плотность). Перевод лежит в
`apps/fridge/units.py` и зовётся из обоих концов — из `_subtract_fridge` при
сборке списка покупок и из `write_off_menu_item` при «приготовил». Одно правило
намеренно: разойдись они, список покупок разошёлся бы с холодильником молча.

Чего НЕТ и не будет: догадок. Нет записи о весе — нет перевода, позиция ведёт
себя как раньше и честно уходит в нехватку. Средний вес «по категории» не виден
никому, а уносит из холодильника не то количество — человек узнает об этом,
только открыв дверцу.

Заполнение — работа с данными, и без неё код ничего не меняет. Для неё есть
команда `mg_unit_weights`: без аргументов показывает, каких пар «товар +
единица» не хватает **прямо сейчас** (смотрит на живые холодильники, а не на
весь каталог), `--set "41=шт:55"` пишет руками, `--ai` оценивает моделью с
dry-run по умолчанию. Плюс раздел «Вес единицы» в карточке товара в админке.

## Новые задачи (chat-85)

### T-34 — глава семьи не мог вести дневник участника · severity: medium · ✅ CLOSED (MG_HEADKEEPS, chat-85)

Ребёнок дневник сам не ведёт, у пожилого не всегда получается, и единственным
выходом было отдать свой пароль. Теперь глава семьи вносит за участника еду,
воду, вес и обхваты: запись принадлежит участнику, но помечена автором
(`added_by`) — в списке корона, рядом имя. Пустое поле значит «внёс сам», и все
прежние записи выглядят как раньше.

Граница проведена по действию, а не по роли. **Добавлять** глава может всегда:
ничего чужого не переписывается. **Править и удалять** записи участника — только
с его согласия (`profile.head_may_edit_diary`) или если он несовершеннолетний
(по `birth_year`; год не указан — считаем взрослым, иначе неизвестный возраст
открывал бы чужой дневник по умолчанию). Согласие даёт сам участник и только
через свой профиль: в `ProfileUpdateSerializer` семьи поля нет намеренно.

Исключение, найденное тестом: свою же запись автор правит и убирает без
разрешения — иначе ошибка главы застревала бы в чужом дневнике навсегда.

Правило целиком в `apps/diary/access.py` — одним местом, как выбор семьи
(MG_ONEFAMILY): у правила о приватности не должно быть копий.

**Попутно в приложении:** участник не доходил до сервера при добавлении еды и
воды за него — событие его несло, запрос нет. Глава добавлял запись в чужой
дневник, а она ложилась в его собственный день, молча.

### T-35 — замеры тела и диаграммы · severity: low · ✅ CLOSED (MG_BODYSIZE + MG_BODYCHART, chat-85)

Обхваты шеи, груди, под грудью, талии и бёдер — таблица по датам
(`BodyMeasurement`), а не пять полей в профиле: профиль хранит текущее
состояние, а вопрос «что происходит» требует прошлых точек. Вес на диете
неделями стоит, а талия в это время уходит — по одному числу человек решает,
что старается зря. Устройство повторяет `WeightLog`: владелец — человек, место
записи — семья, одна строка на дату, запись переживает уход из семьи.

Две диаграммы по кнопке (выбор помнится в localStorage / SharedPreferences):
график веса и контур тела — мужской или женский по профилю — с линиями
обхватов и изменением с прошлого замера. Обе рисуются своим SVG и
CustomPainter: библиотека графиков весит больше, чем весь экран дневника.

Ручка: `GET/POST /api/v1/diary/measurements/` с тем же `?member_id=`, что и у
веса.


### T-30 — регистр названий: правило портило бренды · severity: medium · ✅ CLOSED (MG_CANONCASE2, chat-85)

Канонизация ингредиентов приводила название к «Первая буква заглавная, дальше
строчные». На проде это касалось 18 записей, и у шести из них так ломался
бренд: «Камамбер Vitalat» → «Камамбер vitalat», «Паста Aroy-d карри», «Рис
Arborio», «Соус сладкий чили Zero», «Яица - 1 шт. С1», тесто `"Тестов"` в
кавычках. Латиница, цифра и кавычки — механические признаки имени
собственного; слова с ними правило больше не трогает.

Разовая правка уже лежащего в каталоге — команда `mg_fix_product_case`
(dry-run по умолчанию, `--apply`, `--merge` для слияний). На проде переименовано
12 названий, слияний не потребовалось.

**Найдено dry-run'ом, до записи:** первая версия команды считала имя занятым при
любом совпадении и собиралась слить четыре сыра в записи из выгрузки
OpenFoodFacts (штрих-код, ноль ссылок, ни категории, ни КБЖУ). У машинных
записей, что уехали бы в них, — 9 и 8 связей рецептов, холодильник и три позиции
списка покупок; всё это переехало бы на записи, скрытые из подборщиков
(`visibility.HIDDEN_FROM_PICKERS`). Имя занимают только живые записи общего
каталога; штрих-кодные и семейные — отдельные пространства имён.

### T-31 — `merge_product_into` удалял позиции меню · severity: high · ✅ CLOSED (MG_MERGEALL, chat-85)

Перепривязка ссылок при слиянии продуктов шла по списку из трёх моделей,
написанному до MG_PRODDISH. У `menu.MenuItem.product` и
`menu.ConstructedMealItem.product` стоит CASCADE, поэтому слияние удаляло
позицию меню вместе с дублем — молча, без ошибки. Касалось и `dedup_products`,
который на проде уже запускали: потери задним числом не найти и не восстановить.
Список ссылок теперь строится из `_meta`.

### T-32 — меню с вышедшим сроком не уходили в архив · severity: medium · ✅ CLOSED (MG_MENUEXPIRE, chat-85)

Меню оставалось `active` навсегда: статус меняла только кнопка «в архив»,
которой ни в вебе, ни в приложении нет. В списке выбора копились прошлогодние
меню.

Решение: ночная задача `apps.menu.tasks.archive_expired_menus` (beat, 00:20
местного времени) проставляет `archived` тем, у кого `end_date < сегодня`. День
местный, а не UTC: «срок вышел» — про календарь человека. Список актуальных при
этом отсекает просроченные **по дате**, а не по статусу, — выдача не зависит от
того, дожил ли beat до утра.

Архив — не удаление: меню открывается по ссылке и лежит в `/menu/?archived=true`.
Веб — переключатель «Актуальные / Архив» на странице меню; приложение — кнопка
в шапке, заголовок «Архив меню» и пометка «Архив ·» в закрытом дропдауне. Туда же
попадает меню, восстановленное из карантина задним числом.

### T-33 — последние строки уходили под системную полосу навигации · severity: medium · ✅ CLOSED (MG_NAVBARINSET, chat-85)

Замечено на телефоне: внизу экрана рецепта последние строки читались «как будто
в дымке». Не тема и не градиент: с targetSdk 35+ Android рисует приложение во
весь экран, включая полосу навигации, и кладёт поверх неё полупрозрачную
подложку. Выключить нельзя — `windowOptOutEdgeToEdgeEnforcement` на Android 16
не работает, а targetSdk у нас 36.

У экранов-вкладок отступ снизу берётся из нижней панели, которую держит
Scaffold. У экранов, открываемых поверх, панели нет — и текст под конец страницы
оказывается за подложкой. Поправлено на всех двенадцати таких экранах:
`SafeArea(top: false)` вокруг тела экрана (сверху отступ уже даёт AppBar).

Сборка 1.0.9 (код 21).

## Новые задачи (chat-84)

Все четыре найдены из одного случая: глава семьи пригласил дочь, у дочери на
телефоне не появилось ничего. Замер на проде показал, что дело не в уведомлениях.

### T-25 — приглашение в семью не работает ни у кого · severity: high · ✅ CLOSED (MG_ONEFAMILY, chat-84)

Вопрос «в какой семье пользователь» решается в проекте **восемью независимыми
копиями**, и две из них отвечают иначе, чем остальные шесть:

| место | правило |
|---|---|
| `apps/family/views.py:15` `_get_user_family` | сначала членство, потом владение |
| `apps/fridge/views.py:31` `_get_family` | только членство |
| `apps/fridge/visibility.py:17` `family_of` | только членство |
| `apps/menu/views.py:37` `_get_family` | только членство |
| `apps/payments/views.py:24` `_get_family` | только членство |
| `apps/subscriptions/permissions.py:59` `get_user_family` | только членство |
| `apps/shopping/permissions.py:7` `get_user_family` | **сначала владение** |
| `apps/users/deletion_views.py:72` `_family_of` | **сначала владение** |

Плюс девятая точка — `apps/diary/views.py:29` `_get_member`, она ищет не семью, а
членство, и тем же способом.

Все они берут `.first()` у запроса без сортировки. Это НЕ «как повезёт»: Django
у неотсортированного запроса сам добавляет `order_by("pk")`
(`django/db/models/query.py:1052`), то есть побеждает членство с наименьшим
номером — самое старое.

Собственная семья заводится в момент регистрации
(`apps/users/views.py:256` `_bootstrap_user`). Значит строка своего членства
всегда старше строки любого приглашения, полученного позже, и **выигрывает
всегда**. Приглашение существующего пользователя не даёт ему ничего: ни общего
холодильника, ни меню, ни списка покупок, ни премиума семьи. Работают только
карточки «Без приглашения» (`is_managed`) — у них членство единственное.

Диагностика (прод, user 46 — приглашён в семью 24, владеет семьёй 43):

```
   членство 45 | семья 43 Семья САШУЛЯ | роль head   <-- эту видит приложение
   членство 46 | семья 24 Семья Ольга  | роль member
   семья 24: премиум active до 2036, квота меню безлимит
   семья 43: free, квота меню 4/мес
   все девять точек           → семья 43
   премиум, как его видит сервер: False
```

Масштаб на проде: людей с более чем одним членством — **1** (тот самый случай).
Приглашениями до сих пор никто не пользовался, поэтому баг и не всплывал.

Разовое лечение данных выполнено вручную (см. ниже), но код не тронут.

Патч (MG_ONEFAMILY): правило вынесено в `apps/family/selection.py` —
`current_family()` и `current_membership()`. Все девять точек делегируют туда,
сортировка выписана явно. Поведение сохранено дословно: побеждает самое старое
членство, при отсутствии членств — семья во владении. Две перевёрнутые копии
(`shopping`, удаление аккаунта) приведены к варианту большинства; разница видна
лишь у того, кто владеет семьёй, но членства в ней не имеет — на проде таких нет.
Тесты `apps/family/tests/test_selection.py` проверяют главное свойство: все
девять точек называют одну и ту же семью при одном членстве, при двух, при
владении без членства и при отсутствии семьи.

Само приглашение этим ещё не чинится: какое членство считать рабочим, решает
T-26. Менять правило на «самое новое» до неё нельзя — это молча переселило бы
человека в чужую семью без спроса, ровно то, от чего отказались в T-27.

### T-29 — копий правила выбора семьи оказалось десять, а не восемь · severity: high · ✅ CLOSED (MG_ONEFAMILY, chat-84)

Найдено на проде уже после выкатки. Дочь переключилась в семью с оплаченным
премиумом: всё приложение показывало премиум — и `/users/me/`, и доступ к
разделам, — а страница «Подписка» писала «Активный тариф: Бесплатный» со сроком
её собственной семьи.

Причина: `apps/subscriptions/views.py` имел собственную копию правила, десятую по
счёту. Она не знала про выбор семьи человеком и брала самое старое членство. Цена
ошибки не в надписи: та же функция выбирает семью при оплате и при активации
промокода, то есть платёж мог уйти не в ту семью, за которую человек платит.
Ещё три копии нашлись в кабинете специалиста (`apps/specialists/views.py`) —
«моя семья» при приглашении специалиста, ответе на назначение и списке своих
специалистов.

Как я их пропустил в T-25: копии искал по выводу grep, а вывод был обрезан по
`head -60`, и подписки в него не попали. Проверка глазами по обрезанному
списку — не проверка.

Патч: четыре места переведены на `current_family()` / `current_membership()`.
Плюс сторож `apps/family/tests/test_selection_is_the_only_rule.py` — тест по
тексту исходников, который падает, если в проекте снова появится выражение вида
`FamilyMember.objects…filter(user=…)…first()` где-либо кроме `selection.py`.
Списки всех семей человека (`values_list("family_id")`) он не трогает: это
законный другой вопрос. Оба теста проверены на старом коде — падают, значит ловят
именно этот сбой.

Регрессия закрыта тестами `apps/subscriptions/tests/test_subscription_follows_active_family.py`:
после переключения `/subscriptions/current/` отвечает подпиской новой семьи,
после возврата — своей, и два экрана про премиум больше не расходятся.

### T-26 — переключение активной семьи · severity: medium · ✅ CLOSED (MG_ACTIVEFAMILY, chat-84)

Решение владельца: человек может состоять в нескольких семьях, но в каждый
момент работает в одной. После принятия приглашения интерфейс переключается на
приглашающую семью; обратный путь — переключиться на свою, при этом чужая
перестаёт отображаться до обратного переключения. При исключении из семьи
доступ теряется и остаётся только своя.

Никакого понятия «текущая семья» в коде нет — ни поля, ни ручки, ни
переключателя. Зато есть готовый образец «работать не от своей семьи»: у
специалистов уже принимается `?family_id=` (`apps/fridge/views.py:36`
`_family_for`, `apps/shopping/permissions.py:43` `resolve_write_family`).

Клиентам это почти ничего не стоит: ни мобильное приложение, ни веб нигде не
передают family_id — они спрашивают `/family/`, а решает сервер. Значит хранить
выбор надо на сервере (поле у пользователя), а в клиентах добавить переключатель
и сброс кэшей.

Сделано на сервере (MG_ACTIVEFAMILY): у пользователя появилось поле
`active_family`, и `family/selection.py` читает его первым — поэтому переключение
меняет разом все девять точек, ради чего T-25 и делалась. Указатель учитывается,
только пока человек состоит в этой семье: проверка идёт при каждом чтении, так
что исключённый молча возвращается к своей семье, а не упирается в закрытую дверь
с ошибкой на каждом экране. Ручка исключения вдобавок снимает указатель сразу.

Две ручки: `GET /family/choices/` — где я состою, с пометкой активной, своей,
ролью, числом участников и наличием премиума у каждого стола; `POST
/family/switch/` — сесть за другой стол, только за тот, где состоишь (чужой
отвечает 404 и не подтверждает существование). Ответ переключения — тот же
список, чтобы клиент не слал второй запрос.

Пометка `has_premium` в списке нужна не для красоты: премиум оплачивается за
стол, и при возврате в свою семью он пропадает. Это выглядит как «отобрали
оплату», поэтому предупредить надо до переключения, а не показать пропажу после.

Клиенты. На экране «Семья» появился список столов с пометкой активного и
кнопкой «Перейти» — показывается, только когда семей больше одной.

Сброс состояния оказался важнее самого переключателя. Холодильник, меню, покупки
и дневник держат своё состояние, а кэш GET-ответов лежит в SharedPreferences, и
ключи в нём про семью ничего не знают: путь запроса одинаков для любой. Без
сброса человек увидел бы данные семьи, из которой уже вышел, — то есть чужие.
Обходить разделы по одному значило бы однажды забыть какой-то из них, поэтому в
вебе страница перезагружается целиком, а в приложении оба кэша чистятся и дерево
пересобирается (`core/app_restart.dart`): блоки создаются заново и идут на
сервер, процесс живёт, выходить из аккаунта не нужно.

Проверено: 1276 тестов бэкенда, 208 тестов веба, tsc и сборка веба, Flutter CI
run 203 — тесты и debug APK зелёные.

Побочные следствия, которые надо принять сознательно:
подписка и квоты живут на семье, поэтому переключение меняет премиум-статус
человека и то, из чьей квоты тратятся генерации меню; пустая своя семья остаётся
у каждого приглашённого навсегда и будет видна в переключателе; глава не может
выйти из своей семьи (`apps/family/views.py:212` запрещает удалять роль head, а
`Family.owner` обязателен) — для сценария «своя семья больше не нужна» нужна
передача владения или удаление семьи.

### T-27 — приглашение в семью без согласия приглашаемого · severity: medium · ✅ CLOSED (MG_FAMINVITE, chat-84)

`FamilyInviteView` (`apps/family/views.py:46`) называется «пригласить», но
приглашения не существует: сервер находит существующий аккаунт по e-mail или
телефону и сразу создаёт `FamilyMember`. Ни подтверждения, ни уведомления, ни
возможности отказаться. Человек получает общий холодильник и список покупок, а
глава семьи — право видеть и править его профиль и нормы КБЖУ
(`FamilyMemberUpdateView`, `apps/family/views.py:222`).

Решение владельца: согласие нужно.

Образец в проекте уже есть — `ShoppingListAccess` (`apps/shopping/models.py:105`,
MG_SHAREACCEPT): статусы pending/accepted/rejected, ручки `/shopping/pending/` и
`/shopping/lists/<id>/respond/`, экран входящих в приложении
(`shopping_bloc.dart:258`). Приглашение в семью списывается с него почти
дословно.

Отдельная сложность — доставка. Пушей в проекте нет: `flutter_local_notifications`
умеет только локальные напоминания о взвешивании, а серверную таблицу
`Notification` (`apps/notifications/models.py:6`) мобильное приложение вообще не
показывает. Реальные каналы: письмо и мессенджер по `chat_id` из
`PhoneVerification` — тот же, что используется для восстановления пароля
(`apps/users/password_reset.py`). Иначе приглашение увидит только тот, кто сам
откроет приложение и заметит значок.

Сделано на сервере (MG_FAMINVITE): модель `FamilyInvite` с состояниями
ожидает/принято/отклонено/отозвано, строка одна на пару «семья + приглашённый»
(повторное приглашение возвращает её в ожидание, история отказов не копится —
знание «мне уже отказывали трижды» лучше не давать никому). `POST
/family/invite/` теперь зовёт, а не зачисляет; членство появляется только после
согласия. Ручки: `GET /family/invites/` — мои входящие, `POST
/family/invites/<id>/respond/` {accept} — ответить, `DELETE
/family/invites/<id>/` — глава передумал. Принятие сразу сажает человека за этот
стол (MG_ACTIVEFAMILY): он только что сказал «да». Лимит участников по тарифу
считается дважды — при приглашении предупредительно и при согласии заново, семья
могла набраться, пока приглашение лежало.

Доставка — `apps/family/invites.py`: запись в `Notification` (есть всегда),
письмо (если есть почта) и сообщение в мессенджер (если регистрировался по
телефону). Ни один канал не считается обязанным сработать: приглашение уже в
базе, а ронять ответ главе семьи из-за недоехавшего письма нельзя — он решит,
что не пригласил, и нажмёт ещё раз.

В списке участников семьи глава теперь видит `pending_invites` — кого позвали и
кто ещё не ответил. Без этого приглашение выглядело бы как «нажал, и ничего не
произошло».

Клиенты. Входящие приглашения показываются первым блоком на экране «Семья» — с
объяснением, что станет общим и что останется личным, и кнопками «Принять» /
«Отклонить». В карточке семьи глава видит строку «Приглашены» с кнопкой отзыва.
Текст успеха приглашения переписан: раньше он утверждал, что человек уже
приглашён, а значит в семье. В форме приглашения добавлено, что человек решает
сам.

Проверено: 1276 тестов бэкенда (13 новых на приглашения), 208 тестов веба, tsc и
сборка веба, Flutter CI run 203 — тесты и debug APK зелёные.

### T-28 — дневник, вода и вес привязаны к членству, а не к человеку · severity: medium · ✅ CLOSED (MG_OWNDIARY, chat-84)

`DiaryEntry.member`, `WaterLog.member`, `WeightLog.member`
(`apps/diary/models.py:17,49,70`) ссылаются на `FamilyMember`. Членство меняется
вместе с семьёй, значит после переключения (T-26) вся личная история еды, воды и
веса исчезает с экрана и подменяется другой, пустой. Для веса и воды это просто
неверно по существу: данные личные, а лежат в семейной привязке.

Нашлась и вторая беда, уже действующая, а не будущая: `FamilyMember` удалялся
каскадом, то есть **исключение человека из семьи стирало ему весь дневник, всю
воду и все замеры веса**. Ради этого одного стоило чинить.

Патч (MG_OWNDIARY): у всех трёх таблиц появился обязательный `user` — владелец,
а `member` стал необязательной пометкой «за каким столом записано» с SET_NULL.
Уникальность воды и веса перешла с `(member, date)` на `(user, date)`: день у
человека один, за сколькими бы столами он ни сидел. Миграции 0008 (схема) →
0009 (перенос владельца и разбор задвоенных дней, с печатью того, что удалено) →
0010 (поле обязательное, новая уникальность). Права на запись сверяются с
человеком, а не с членством, поэтому своя запись остаётся своей и после ухода из
семьи. Граница видимости выписана в `_scope()`: свой дневник человек видит
целиком, включая записанное в другой семье; чужой (глава семьи, специалист) —
только то, что записано за этим столом. API не изменился, `?member_id=`
работает как раньше — клиенты трогать не пришлось.

Известное ограничение: специалист привязан к семье, поэтому замеры веса клиента,
сделанные тем в другой семье, в кабинет специалиста не попадут. Пока переключения
семей нет (T-26), случай недостижим; после неё — решить, считать ли вес клиента
по человеку целиком.

**Разовое лечение данных на проде (chat-84), кода не касается:** удалить пустую
автосозданную семью 43 «Семья САШУЛЯ» (владелец 46) — тогда все девять точек
сойдутся на семье 24 и премиум семьи станет виден. Удалять только после
проверки, что в семье 43 нет ни меню, ни холодильника, ни записей дневника.
Цена решения: своя семья заводится только при регистрации, поэтому если
пользователя 46 когда-нибудь исключат из семьи 24, своей у него не окажется —
завести придётся руками.
...
### T-23 — чистка say7: удалены 311 негодных к публикации · CLOSED (chat-83)
Из неопубликованного остатка say7 удалены рецепты, у которых нет ни веса
порции (или калорий на порцию), ни фотографии. Такой рецепт требовал и съёмки,
и ручной правки — двойной работы на каждую строку, — и попадал в списки для
фотографа, где эта работа терялась впустую.

Удалено 311: bakery 112, soup 78, snack 44, dessert 30, main 27, sauce 11,
breakfast_dish 5, drink 2, salad 2. Вместе с ними ушли 2578 связей
RecipeProduct — хвост удалённых рецептов.

Проверено сразу после операции: опубликованные не тронуты (side 426, main 170,
salad 57, breakfast_dish 54, dessert 45, bakery 35, snack 30, soup 19 — цифра в
цифру как в замерах до), позиций меню каскадом не задето ни одной.

**Что это дало.** В остатке say7 (1082 рецепта) больше нет непубликуемых:
у всех есть вес порции и калории, не хватает только фотографии. Любой снимок
теперь даёт публикуемый рецепт.

Решение по супам принято сознательно и вопреки моему возражению: 78 супов без
веса удалены вместе с остальными, хотя супы — единственная роль с дефицитом и
вес им можно было назначить соглашением одной командой. Формулировка
пользователя: «супы найдём, это не проблема».

Инструменты, оставшиеся от этой задачи (`mg_export_recipes_xlsx` с флагами
`--unpublished --missing-portion --no-photo --exclude-dish`, стыковка с
`delete_recipes_from_csv`), пригодны для следующих импортов как есть.

### T-24 — s1: завтраки в топе повторяемости · severity: low · ✅ CLOSED, чинить нечего (chat-83)
В замере после публикации салатов пять строк из пятнадцати в топе — завтраки, с
частотой 30–40 %. Пул `breakfast_dish` 54 при семи слотах в неделю даёт
ожидаемые ~13 %. Втрое выше, чем следует из размера пула, — значит до слота
доходят не все.

Гипотеза (НЕ проверена, проверять командой ниже): та же природа, что у T-22.
Завтрак — основной приём, а в основные приёмы не пускаются блюда с
`has_added_sugar=True`, если есть хоть один кандидат без него. В `SWEET_ROLES`
мы внесли только десерт и выпечку, `breakfast_dish` там нет. Значит сладкие
завтраки — сырники, панкейки, оладьи — могут не проходить вовсе.

Косвенно подтверждается составом топа: все пять несладкие (яйцо пашот, вафли с
сыром и цукини, кукурузные вафли, вафли со спаржей и яйцом, кабачковые вафли).
...
qs = Recipe.objects.filter(is_published=True, dish_type='breakfast_dish')
print('завтраков:', qs.count(), '| без сахара:', qs.filter(has_added_sugar=False).count())
"
```

**Проверено: гипотеза верна по направлению, но не по силе, и вопрос закрыт.**
Несладких завтраков 31 из 54 — правило сокращает пул вдвое, а не в пять раз,
как было с десертами (3 из 39).

Дальше вопрос закрывает арифметика. Семь слотов завтрака в неделю на двадцать
прогонов — 140 выборок из 31 рецепта, ожидаемая частота 22–23 %. Максимум среди
31 рецепта при таком раскладе как раз и приходится примерно на 8 прогонов из
20, то есть наблюдаемые 40 % — обычный разброс случайного выбора, а не отсев.

Чинить нечего. Для завтрака запрет на сахар осмыслен — это осознанное правило
про здоровье, в отличие от десерта, где оно отрицало само себя. Флаг
`has_added_sugar` вдобавок расставлен разумно: среди несладких есть и панкейки,
и тыквенные вафли — сладкие на вкус, но без добавленного сахара.

Оставлено в заделе как разобранный след: следующий, кто увидит завтраки в топе,
не должен тратить время на ту же проверку.
...
### T-17 — s1: дополнить пулы рецептов salad/snack/bakery · severity: medium · ✅ CLOSED (chat-83)
...
**ЗАКРЫТО (chat-83).** Сняты фотографии и опубликованы 77 рецептов: 10 супов и
67 перекусов. Опубликовано всего 836 → 913. Супы 19 → 29 при потребности 28,
перекусы 30 → 97 при 56 — обе роли, из-за которых задача заводилась, зелёные.
Пятеро перекусов с фото, но без веса порции придержаны (они в T-20).

Замер `mg_analyze_s1_repeats --runs 20 --days 7` по трём точкам:

| | до правок генератора | после правок | после публикации |
|---|---|---|---|
| разных рецептов встретилось | 223 | 269 | **273** |
| самый частый | 100 % | 65 % | **55 %** |
| в ≥50 % прогонов | 16 | 7 | **2** |
| в ≥80 % прогонов | 7 | 0 | **0** |
| жёлтых ролей | 2 | 2 | **0** |

**Салаты добраны (chat-83, тот же день).** Сняты фотографии и опубликованы ещё
92 салата: 57 → 149, роль вышла на «отлично» (порог 140). Опубликовано всего
913 → 1005. Проверка перед публикацией показала ноль недостижимых по разметке
приёмов — салат роль с выбором приёма, и для него фильтр `suitable_for`
действует, в отличие от супов и десертов.

Итоговый замер после всех публикаций:

| | до правок генератора | после правок | +супы и перекусы | +салаты |
|---|---|---|---|---|
| разных рецептов встретилось | 223 | 269 | 273 | **338** |
| самый частый | 100 % | 65 % | 55 % | **50 %** |
| в ≥50 % прогонов | 16 | 7 | 2 | **1** |
| в ≥80 % прогонов | 7 | 0 | 0 | **0** |

Пулы: side 426, main 170, salad 149, snack 97, breakfast_dish 54, dessert 45,
bakery 35, soup 29. Все роли зелёные, main и salad — «отлично».

**Что осталось.** Супы: 29 при пороге 28, то есть ровно в минимум «хорошо» —
восемь строк из пятнадцати в топе принадлежат им, и это честная арифметика
(семь слотов в неделю на 29 рецептов). Лечится только новыми супами: в остатке
say7 их не осталось, нужен новый источник.

Завтраки в топе (пять строк из пятнадцати) разобраны и оказались нормой —
см. T-24.
...
### D-09 — Таблица символов нативного кода для Play · severity: low · ✅ CLOSED (MG_NDKSYMBOLS, chat-84)
...
Сделано в релизе 1.0.8: `ndk { debugSymbolLevel = "SYMBOL_TABLE" }` в
`defaultConfig`. SYMBOL_TABLE, а не FULL: имена функций для разбора стека дают
оба, а FULL кладёт ещё и номера строк чужих исходников, которых у нас всё равно
нет, и заметно утяжеляет загрузку.

### D-10 — Устаревший `pubspec.lock` мобильного приложения · severity: low · ✅ CLOSED (chat-84)
...
Сделано в релизе 1.0.8. Машины с Flutter под рукой не было, поэтому лок снят с
прогона CI: временный шаг напечатал разрешённый лок в лог, оттуда он и попал в
репозиторий. Разошлось прилично — добавились `flutter_local_notifications` (с
двумя платформенными пакетами), `open_filex`, `package_info_plus`, `timezone`,
`mobile_scanner` 5.2.3 → 6.0.11, `web` 0.5.20 → 0.5.42.

Чтобы лок больше не отставал молча, временный шаг заменён постоянной проверкой:
после `flutter pub get` сборка падает, если лок разошёлся, и показывает
расхождение. Комментарий в `flutter_ci.yml`, утверждавший, что лока в
репозитории нет, исправлен — именно эта неправда и была причиной, по которой его
не обновляли.
