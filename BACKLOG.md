...
### T-24 — s1: завтраки повторяются втрое чаще, чем следует из пула · severity: low · OPEN, ГИПОТЕЗА НЕ ПРОВЕРЕНА (chat-83)
...
**Отличие от T-22, из-за которого это НЕ чинится тем же способом.** Для десерта
запрет на сахар был бессмыслен: десерт сладкий по определению, и правило
отрицало само себя. Для завтрака запрет осмыслен — это осознанное правило про
здоровье, и снимать его значит менять продуктовое решение, а не чинить ошибку.
Поэтому решение за пользователем, а не техническое.
...
И отдельное наблюдение — см. T-24 про завтраки.
