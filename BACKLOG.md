...
### T-21 — s1: разметка приёмов разошлась с ролями генератора · severity: medium · ✅ CLOSED (chat-83)
...
Как чинили — и почему не разметкой. Напрашивалось дописать `lunch` десертам и
выпечке: сухой прогон по dev показал 630 таких записей (выпечка 399, десерты
212). Но то же поле читает фильтр «приём пищи» в каталоге рецептов
(`apps/recipes/filters.py`), и человек, выбравший «обед», получил бы в выдачу
четыреста наименований пирогов и маффинов. Разметка отвечает на свой вопрос —
«когда это едят» — правильно; неправильно было спрашивать её там, где выбора
нет.

Поэтому правка в генераторе (решение согласовано, chat-83):

* `ROLES_WITH_MEAL_CHOICE` в `apps/menu/generator.py` — роли, встречающиеся
  больше чем в одном приёме. Фильтр по `suitable_for` применяется только к ним:
  у роли с единственным слотом пометка не выбирает между слотами, а вычёркивает
  рецепт целиком. Набор ВЫВОДИТСЯ из `MEAL_COMPONENTS`, а не выписан рядом —
  копия разошлась бы с раскладкой ровно так же, как разошлась разметка импорта.
* Применено в двух местах подбора: `_pick_for_role` (s1) и `_pick_role_addon_s2`.
  В `_pick_plate_s3` не трогали: там пулы плитки (protein/carb/veg), а не роли
  приёма, и находка к нему не относится.
* Данные НЕ меняются: 630 записей остаются как есть, каталог прежний, будущие
  импорты чинить не придётся вовсе.
* `import_say7_recipes.SUITABLE_BY_DISH` оставлен как был; в комментарии
  записано, почему дописывать туда «обед» не надо.
* Тест `apps/menu/tests/test_suitable_for_single_slot.py`: набор ролей выводится
  из раскладки; десерт с пометкой «перекус» попадает в обед; у роли с выбором
  приёма фильтр продолжает работать.

Проверено на dev после выкатки: из 39 десертов встретилось 33 — пул стал
доступен целиком, недостижимости больше нет.

**Но нашёлся второй, независимый отсев — см. T-22.**

### T-22 — s1: запрет сахара в основных приёмах убивал десертный слот · severity: medium · ✅ CLOSED (chat-83)
Найден тем же прогоном, что и T-21, но причина другая и независимая. После
починки T-21 на dev три десерта по-прежнему стояли в 20 прогонах из 20, а весь
остальной хвост — по 1–6.

Правило MG_502_503: в основные приёмы (завтрак, обед, ужин) нельзя блюда с
`has_added_sugar=True`, если есть хоть один кандидат без него. Десертная роль
живёт в обеде, обед — основной приём. Значит в десертный слот проходили только
десерты БЕЗ добавленного сахара.

Замер на dev: таких три из 39 (4000 «Шарлотка без сахара», 4013 «Бискотти»,
5351 «Галета с клубникой») — ровно те, что стояли в каждом меню. Пока они не
израсходованы за прогон, остальные тридцать шесть не рассматривались вовсе; на
четвёртый-пятый день несладкие кончались, и сахарные наконец проходили. У
выпечки то же самое, мягче: 7 несладких из 19.

Природа та же, что у T-21: правило писалось, когда десертной роли в обеде не
было (она пришла с RB001_V_step4), и после её появления стало отрицать само
себя — десерт по определению сладкий.

Ложный след, чтобы не проверяли второй раз: калорийное окно
(`MEAL_CAL_WINDOW_TIERS`) тут ни при чём. У синтетического члена семьи в
`mg_analyze_s1_repeats` цель по калориям пустая, а при пустой цели окно вообще
не применяется.

Сделано: `SWEET_ROLES = {"dessert", "bakery"}` в `apps/menu/generator.py`,
запрет на сахар их не касается. Сахара в дне больше не станет:
`DESSERT_MAX_PER_DAY` держит десерт и выпечку вместе на одной штуке в сутки, а
`SWEET_PER_DAY_LIMIT` продолжает работать для перекусов. Снято не ограничение
количества, а требование, чтобы сладкое блюдо было несладким. Тесты: сладкий
десерт берётся в обед, сладкий и несладкий конкурируют, а сладкое ОСНОВНОЕ
блюдо правило по-прежнему не пускает.

Проверено на dev после выкатки: встретилось **38 десертов из 39**, максимум
40 % (8 прогонов из 20), «в ≥50 % прогонов: 0», «≥80 %: 0». Распределение
сплющилось — вечных десертов больше нет.

Заодно починена ложная тревога самого анализатора: строка «⚠ ПРОБЛЕМА» горела
даже при нулевой повторяемости, потому что второе условие (`avg_distinct <
days*2`) подобрано для меню целиком, а при `--role` в выборке остаются слоты
одной роли — у десерта их ровно семь на неделю, то есть 7 против порога 14, и
тревога срабатывала всегда. Теперь это условие считается только для полного
прогона.

Замер на ПРОДЕ до и после выкатки обеих правок (T-21 + T-22):

| | до | после |
|---|---|---|
| десертов встретилось из 45 | **8** | **44** |
| десерты в ≥50 % прогонов | 7 | **0** |
| десерты в ≥80 % прогонов | 6 | **0** |
| разных рецептов в меню целиком | 223 | **269** |
| самый частый рецепт | 100 % | **65 %** |
| в ≥80 % прогонов (все роли) | **7** | **0** |
| вердикт анализатора | ⚠ проблема | ✓ норма |

Сорок шесть рецептов, которых генератор не доставал вовсе, вошли в оборот.
Десерты ушли из топа повторяемости полностью.
...
**Итог после T-21 и T-22 (chat-83, замер на проде).** Оба отсева сняты, картина
наконец неискажённая. В топе повторяемости остались супы (8 строк из 15) и
салаты (7); десертов там нет вовсе.

| роль | пул | need (good) | дефицит |
|---|---|---|---|
| soup | 19 | 28 | **+9** |
| snack | 30 | 56 | **+26** |
| salad | 57 | 56 | закрыто, но без запаса |
| bakery | 35 | 28 | закрыто |
| main / breakfast_dish / dessert | — | — | зелёные |

Супы: семь слотов в неделю на девятнадцать рецептов — каждый обязан появляться
примерно в 37 % прогонов, наблюдаем 40–65 %. Это арифметика пула, а не отсев.

Остаток задачи: **супы +9 и перекусы +26**. Салаты и выпечку добирать больше не
нужно — их закрыла публикация say7 (T-20) вместе со снятием отсевов.
