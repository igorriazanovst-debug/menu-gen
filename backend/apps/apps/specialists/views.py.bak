...
class CabinetClientListView(APIView):
    ...
    def get(self, request):
        ...
        ).select_related("family").prefetch_related("family__members__user")

        families = [a.family for a in assignments]
        serializer = ClientFamilySerializer(
            families, many=True, context={"specialist": specialist}
        )
...
class CabinetClientMenuDetailView(APIView):
    ...
    def get(self, request, family_id, menu_id):
        ...
        try:
            ...
            ).get(id=menu_id, family_id=family_id)
...
class CabinetRecommendationDetailView(APIView):
    ...
    def patch(self, request, family_id, rec_id):
        ...
        serializer = RecommendationWriteSerializer(
            ...
            context={"assignment": rec.assignment}
        )
        serializer.is_valid(raise_exception=True)
        serializer.save()
...
class AssignmentInviteView(APIView):
    ...
    def post(self, request):
        ...
        return Response({"detail": "Приглашение отправлено.", "assignment_id": assignment.id}, status=status.HTTP_201_CREATED)
...
class CabinetPendingAssignmentsView(APIView):
    ...
    def get(self, request):
        ...
        data = [
            {"assignment_id": a.id, "family_id": a.family_id, "family_name": a.family.name}
