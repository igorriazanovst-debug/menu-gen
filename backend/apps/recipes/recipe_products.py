...
def _sentence_case(s):
    ...
    выглядят как две разные базы. Регистр — единственное, что тут можно
    исправить механически и наверняка, поэтому его и исправляем.
    """
    s = (s or "").strip()
    if not s:
        return s
