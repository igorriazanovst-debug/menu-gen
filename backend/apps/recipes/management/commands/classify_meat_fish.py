...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--only-protein", action="store_true", help="ограничиться food_group=protein (по умолчанию — все)"
        )
    ...
    def handle(self, *args, **opts):
        ...
        for r in qs.iterator(chunk_size=500):
            ...
            if new_red != old_red or new_fish != old_fish:
                ...
                if dry and changed <= 30:
                    self.stdout.write(
                        f"  [{r.id}] {r.title[:60]:60} " f"red:{old_red}->{new_red}  fish:{old_fish}->{new_fish}"
                    )
                if not dry:
        ...
        self.stdout.write("")
        self.stdout.write(
            self.style.SUCCESS(f"Итого: red_meat={red_count}  fatty_fish={fish_count}  changed={changed}  dry={dry}")
        )
