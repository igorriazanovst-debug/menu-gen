...
# MG_SUITABLE: здесь отвечают на вопрос «когда это едят», и только на него.
#
# Соблазн дописать «обед» десертам и выпечке был и был отвергнут. Выглядит он
# обоснованно: у генератора роли «десерт» и «выпечка» существуют только в обеде,
# и раньше пометка ["snack"] делала такие рецепты недостижимыми — из 45 десертов
# генератор доставал 5, из 35 выпечек 2 (замер на проде, chat-83).
#
# Но это же поле читает фильтр «приём пищи» в каталоге рецептов
# (apps/recipes/filters.py), и «обед» у выпечки высыпал бы человеку в выдачу по
# обеду четыреста наименований пирогов и маффинов. Разметка отвечает на свой
# вопрос правильно; неправильно было спрашивать её там, где выбора нет.
#
# Поэтому чинили не здесь, а в генераторе: он больше не спрашивает suitable_for
# у ролей с единственным слотом (см. ROLES_WITH_MEAL_CHOICE в
# apps/menu/generator.py). Ставить сюда приёмы «под генератор» не нужно —
# пишите то, что верно по смыслу блюда.
SUITABLE_BY_DISH = {
    "soup": ["lunch", "dinner"],
    "main": ["lunch", "dinner"],
    "salad": ["lunch", "dinner"],
