...
# MG_SUITABLE: приёмы обязаны включать тот, где роль реально встречается.
#
# Раньше здесь стояло `dessert: ["snack"]` и `bakery: ["breakfast", "snack"]`, и
# это выглядело разумно — десерт ведь и правда сойдёт за перекус. Но у генератора
# роли «десерт» и «выпечка» существуют ТОЛЬКО в обеде, а перекус берёт блюда с
# dish_type='snack', а не десерты. То есть разметка объявляла рецепт годным туда,
# куда он попасть не может, и негодным туда, где у него единственный слот.
#
# Цена ошибки (замер на проде, chat-83): из 45 десертов генератор доставал 5, из
# 35 выпечек — 2. Остальные шли только по запасному пути, когда достижимые
# израсходованы, и это читалось как «нехватка пула», хотя пул был.
#
# Сверять руками этот словарь с MEAL_COMPONENTS не нужно и не надо: расхождение
# ловит `manage.py mg_fix_suitable_for` — он выводит таблицу приёмов из самого
# генератора и чинит разметку.
SUITABLE_BY_DISH = {
    "soup": ["lunch", "dinner"],
    "main": ["lunch", "dinner"],
    "salad": ["lunch", "dinner"],
