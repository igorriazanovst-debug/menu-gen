...
DISH_TYPE_KEYWORDS = {
    ...
    "sauce": [r"\bсоус\w*", r"\bзаправк\w*", r"\bмаринад\w*", r"\bподлив\w*", r"\bкетчуп\w*"],
    "dessert": [
        r"\bдесерт\w*",
        r"\bмусс\b",
        r"\bжеле\b",
...
INGREDIENT_GROUPS = {
    ...
    "vegetable": [
        ...
        r"руккол",
        r"салат\b",
        r"\bсалат[аеуы]?\b",  # лист.салат
        r"сельдере",
        r"спарж",
        r"горошек",
        r"кукуруз",  # как овощ-гарнир
        r"шампиньон",
        r"\bгриб[аоуыехм]?\b",
        r"\bопят",
        r"подосинов",
    ...
    "oil": [
        r"масл[оаеу] растительн",
        r"растительн[оы]е масл",
        r"масл[оаеу] подсолнечн",
        r"подсолнечн[оы]е масл",
        r"оливков[оы]е масл",
        r"масл[оаеу] оливков",
        r"кунжутн[оы]е масл",
        r"льнян[оы]е масл",
...
COMPILED = {g: [re.compile(p) for p in pats] for g, pats in INGREDIENT_GROUPS.items()}
...
def to_grams(qty: str, unit: str) -> float:
    ...
    if "кг" in u:
        return q * 1000
    if "грамм" in u or u == "г":
        return q
    if "мл" in u or "литр" in u or u == "л":
        return q
    if "ст. ложк" in u or "ст.л" in u:
        return q * 15
    if "ч. ложк" in u or "ч.л" in u:
        return q * 5
    if "стакан" in u:
        return q * 240
    if "штук" in u or u == "шт":
        return q * 80
    if "зубч" in u:
        return q * 5
    if "пучок" in u or "пучк" in u:
        return q * 30
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--dry-run", action="store_true", help="ничего не менять, показать только статистику")
        parser.add_argument("--limit", type=int, default=0, help="ограничить кол-во рецептов (для отладки)")
        parser.add_argument("--show-samples", type=int, default=0, help="вывести N примеров изменённых рецептов")
    ...
    def handle(self, *args, **opts):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\n{'DRY-RUN ' if dry else ''}готово. Обработано: {sum(stat_old.values())}. " f"Изменено: {changed}."
            )
        )
