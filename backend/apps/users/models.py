...
class Profile(models.Model):
    ...
    # MG_HEADKEEPS: разрешение главе семьи вести чужой дневник.
    #
    # Добавлять записи глава может всегда — они помечены короной, видно, кто
    # внёс, и человек их поправит сам. А вот менять и удалять чужое — другое
    # дело: так молча переписывается то, что человек записал о себе. Поэтому
    # только с его согласия, и согласие даёт он сам.
    #
    # Исключение — несовершеннолетние: у ребёнка дневник ведут родители, ждать
    # от него галочки бессмысленно. Возраст берётся из `birth_year`; неизвестный
    # год рождения считается взрослым — предполагать обратное значило бы открыть
    # чужой дневник по умолчанию. Правило целиком живёт в apps/diary/access.py,
    # чтобы не расползлось копиями, как когда-то выбор семьи (MG_ONEFAMILY).
    head_may_edit_diary = models.BooleanField(
        default=False,
        help_text="Разрешить главе семьи править и удалять мои записи (MG_HEADKEEPS).",
    )
    updated_at = models.DateTimeField(auto_now=True)
