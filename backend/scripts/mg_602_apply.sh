#!/usr/bin/env bash
# MG-602 apply: bugfix views.py + новые тесты test_mg_602_views.py
#
# Что делает:
#  1) Backup views.py
#  2) Bugfix _can_edit_menu (убрать m.can_edit_menu)
#  3) Bugfix _menu_snapshot (добавить meal_slot, component_role, is_cheat_meal)
#  4) Bugfix MenuRestoreView (убрать is_salad=, добавить новые поля)
#  5) Создать apps/menu/tests/test_mg_602_views.py
#  6) Прогнать pytest + coverage
set -eu
...
TESTS="$BACKEND/apps/menu/tests/test_mg_602_views.py"

echo "=========================================="
echo "  MG-602 APPLY  ($TS)"
echo "=========================================="

# ── 0. Sanity ───────────────────────────────────────────────────────────────
[ -f "$VIEWS" ] || { echo "ERR: $VIEWS не найден"; exit 1; }

# ── 1. Backup ──────────────────────────────────────────────────────────────
cp "$VIEWS" "$VIEWS.bak.${TS}"
echo "Backup: $VIEWS.bak.${TS}"

# ── 2. Bugfix через python (атомарная замена блоков) ───────────────────────
python3 <<PYEOF
import io, re, sys
...
# ─── BUGFIX 1: _can_edit_menu ──────────────────────────────────────────────
old1 = '''def _can_edit_menu(user, family):
...
new1 = '''def _can_edit_menu(user, family):
...
if old1 not in src:
    print("ERR: блок _can_edit_menu не найден"); sys.exit(2)
src = src.replace(old1, new1)
print("OK: _can_edit_menu пропатчен")

# ─── BUGFIX 2: _menu_snapshot ──────────────────────────────────────────────
old2 = '''def _menu_snapshot(menu):
    items = []
...
if old2 not in src:
    print("ERR: блок _menu_snapshot не найден"); sys.exit(3)
src = src.replace(old2, new2)
print("OK: _menu_snapshot пропатчен")

# ─── BUGFIX 3: MenuRestoreView — убрать is_salad, добавить новые поля ──────
old3 = '''                    MenuItem.objects.create(
                        menu=menu,
                        recipe=recipe,
                        member=member,
                        meal_type=item["meal_type"],
...
if old3 not in src:
    print("ERR: блок MenuItem.objects.create в MenuRestoreView не найден"); sys.exit(4)
src = src.replace(old3, new3)
print("OK: MenuRestoreView пропатчен")
...
# ── 4. Создание test_mg_602_views.py ───────────────────────────────────────
cat > "$TESTS" <<'TESTEOF'
# MG_602_V_tests
"""
...
# ── 5. AST-проверка теста ──────────────────────────────────────────────────
$COMPOSE exec -T backend python -c "import ast; ast.parse(open('apps/menu/tests/test_mg_602_views.py').read()); print('test_mg_602_views.py: AST OK')"

# ── 6. Прогон pytest нового файла ─────────────────────────────────────────
echo
echo "--- pytest test_mg_602_views.py ---"
$COMPOSE exec -T backend pytest apps/menu/tests/test_mg_602_views.py -v --tb=short 2>&1 | tail -80

# ── 7. Полный прогон + coverage ───────────────────────────────────────────
echo
...
echo
echo "=========================================="
echo "  MG-602 APPLY DONE"
echo "  Backup views.py: $VIEWS.bak.${TS}"
echo "=========================================="
