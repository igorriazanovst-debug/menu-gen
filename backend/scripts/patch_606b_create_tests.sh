#!/usr/bin/env bash
# MG-606.B — создать недостающий test_mg_606b_premium_or_readonly.py + регресс
# Расположение: /opt/menugen/backend/scripts/patch_606b_create_tests.sh
# Запуск: bash /opt/menugen/backend/scripts/patch_606b_create_tests.sh 2>&1 | tee /tmp/patch_606b_create_tests.log
...
TARGET="${BACKEND}/apps/diary/tests/test_mg_606b_premium_or_readonly.py"

echo "===================================================================="
echo "PATCH-606.B (создание тестов) — старт ($TS)"
echo "===================================================================="

# ── 1. Создаём недостающий файл тестов ────────────────────────────────────
cat > "$TARGET" <<'PYEOF'
...
# ── 2. Sanity: видно ли файл внутри контейнера ────────────────────────────
echo
echo "--- проверка видимости файла внутри контейнера ---"
$COMPOSE exec -T backend ls -la apps/diary/tests/test_mg_606b_premium_or_readonly.py

# ── 3. Прогон MG-606.B ────────────────────────────────────────────────────
echo
echo "===================================================================="
echo "Прогон MG-606.B"
echo "===================================================================="
...
echo
echo "PATCH-606.B (создание тестов) — финиш"
