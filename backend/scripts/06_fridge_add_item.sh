#!/usr/bin/env bash
# Fridge add-item: backend (OFF fallback) + mobile + web
# Idempotent where possible. Run from /opt/menugen.
set -eu
...
BAK="$ROOT/backups/fridge_add_${TS}"
...
# =================================================================
echo
echo "===== [BACKEND 1/5] Settings: OPENFOODFACTS_* ====="
SETTINGS_BASE="$BACKEND/menugen/settings/base.py"
if [ ! -f "$SETTINGS_BASE" ]; then
  SETTINGS_BASE=$(grep -rln "DEBUG\s*=" "$BACKEND/menugen/settings/" | head -1)
fi
echo "Settings file: $SETTINGS_BASE"
if ! grep -q "OPENFOODFACTS_BASE_URL" "$SETTINGS_BASE"; then
  cat >> "$SETTINGS_BASE" <<'PYEOF'
...
PYEOF
  echo "  appended OPENFOODFACTS_* to $SETTINGS_BASE"
else
...
s = p.read_text()
if "image_url" in s:
    print("  image_url already in Product")
else:
...
# =================================================================
echo
echo "===== [BACKEND 3/5] Serializers: image_url ====="
...
# =================================================================
echo
echo "===== [BACKEND 4/5] OFF service + extended BarcodeLookupView ====="
...
def fetch_product_from_off(barcode: str) -> Optional[Product]:
    ...
    # Upsert: barcode is unique, race-safe with get_or_create + update
...
class BarcodeLookupView(APIView):
    ...
    Body: {"barcode": "..."}
    1) try local Product
    2) try OpenFoodFacts (cached into local Product table)
    3) 404 with {"detail": ..., "barcode": ...}
    """
    permission_classes = [permissions.IsAuthenticated, IsFamilyPremiumOrReadOnly]
...
# =================================================================
echo
echo "===== [BACKEND 5/5] Tests: OFF fallback ====="
...
# =================================================================
echo
echo "===== [MOBILE 1/6] pubspec.yaml: mobile_scanner + image_picker + permission_handler ====="
python3 - <<'PYEOF'
from pathlib import Path
...
for ln in lines:
    out.append(ln)
    # Insert after cached_network_image
...
# =================================================================
echo
echo "===== [MOBILE 2/6] AndroidManifest: CAMERA permission ====="
python3 - <<'PYEOF'
from pathlib import Path
...
if "android.permission.CAMERA" in s:
    ...
    if marker not in s:
        raise SystemExit("ERROR: INTERNET permission marker not found")
    new = (
        marker
...
# =================================================================
echo
echo "===== [MOBILE 3/6] FridgeBloc: extended events ====="
...
/// Add a new fridge item (manual or from barcode lookup).
...
/// Returns the scanned barcode string (or null on cancel) via Navigator.pop.
...
# =================================================================
echo
echo "===== [MOBILE 5/6] AddFridgeItemSheet (manual + barcode) ====="
...
# =================================================================
echo
echo "===== [MOBILE 6/6] FridgeScreen: pass apiClient + open sheet ====="
...
if old in s:
    ...
    print("WARN: marker for FridgeScreen not found in app_router.dart")
PYEOF
...
echo "  2. cd $WEB && npm start (or rebuild)"
echo "  3. flutter pub get inside $MOB (CI will do this on push)"
echo "  4. Commit & push: backend + mobile + web."
