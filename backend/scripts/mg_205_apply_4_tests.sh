#!/usr/bin/env bash
# MG-205 apply STEP 4/4: pytest-тесты для сценариев auto/user/specialist + reset
# Идемпотентен (проверяет наличие test_mg_205.py).
# Запуск: bash /opt/menugen/backend/scripts/mg_205_apply_4_tests.sh
...
echo "=== MG-205 apply STEP 4 ==="
echo "TS=${TS}"

# ============================================================
# 0. Проверка инфраструктуры pytest
# ============================================================
echo "[0] проверка pytest-инфраструктуры..."
${COMPOSE} exec -T backend bash -c '
  echo "  --- pytest: $(which pytest 2>/dev/null || echo NOT_INSTALLED)"
  pytest --version 2>/dev/null | head -1 || echo "  (pytest не установлен)"
  echo "  --- pytest.ini / pyproject:"
  ls /app/pytest.ini /app/pyproject.toml /app/setup.cfg 2>/dev/null || echo "  (config files not found in /app)"
  echo "  --- conftest.py:"
  find /app -name conftest.py -not -path "*/.git/*" -not -path "*/__pycache__/*" 2>/dev/null | head -10
  echo "  --- DJANGO_SETTINGS_MODULE:"
  echo "  ${DJANGO_SETTINGS_MODULE:-(not set)}"
  echo "  --- pytest-django:"
  pip show pytest-django 2>/dev/null | grep -E "^Name|^Version" || echo "  (pytest-django не установлен)"
' || true

# ============================================================
# 1. Создание тест-файла
# ============================================================
TEST_FILE="${BACKEND}/apps/users/tests/test_mg_205.py"
TEST_DIR="${BACKEND}/apps/users/tests"
echo "[1] создаю ${TEST_FILE}..."

mkdir -p "${TEST_DIR}"
if [ ! -f "${TEST_DIR}/__init__.py" ]; then
  touch "${TEST_DIR}/__init__.py"
  echo "    создан tests/__init__.py"
fi
...
PYEOF
echo "    OK: тест-файл создан, $(wc -l < ${TEST_FILE}) строк"

# ============================================================
# 2. Запуск тестов
# ============================================================
echo "[2] запуск pytest..."
${COMPOSE} exec -T backend pytest apps/users/tests/test_mg_205.py -v 2>&1 | tail -40 || {
  echo
  echo "  pytest вернул ошибку — возможны причины:"
  echo "  1) pytest/pytest-django не установлены в контейнере"
  echo "  2) DJANGO_SETTINGS_MODULE не задан — попробую через manage.py test:"
  echo
  ${COMPOSE} exec -T backend python manage.py test apps.users.tests.test_mg_205 -v 2 2>&1 | tail -40 || true
}
