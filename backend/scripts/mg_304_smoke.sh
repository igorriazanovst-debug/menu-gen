#!/bin/bash
# MG-304 smoke: проверка генерации в API + сохранение warnings.
set -uo pipefail
...
echo
echo "[2] e2e: реальная генерация для admin@dev.local (есть birth_year=1985)"
...
family = Family.objects.filter(members__user__email="admin@dev.local").first() \
         or Family.objects.first()
print("family:", family.id, family.name)
members = list(FamilyMember.objects.filter(family=family).select_related("user__profile"))
print("members:", [(m.id, getattr(m.user, "email", None),
                    getattr(getattr(m.user, "profile", None), "birth_year", None)) for m in members])

start = date.today()
g = MenuGenerator(family=family, members=members, period_days=2,
                  start_date=start, plan_code="free", filters={"meal_plan_type": "3"})
items = g.generate()
...
print(f"items total: {len(items)}, virtual snacks added: {virt}")
print(f"warnings: {g.last_warnings}")
for m in members:
...
echo
echo "[3] HTTP: POST /api/menu/generate/ от admin (если есть)"
...
if not u:
    print("admin@dev.local not found — skipping HTTP smoke"); sys.exit(0)
...
data = resp.json() if resp.status_code < 500 else resp.content
