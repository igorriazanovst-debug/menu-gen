#!/usr/bin/env bash
# MG-604 fix-2: BoomDict должен быть непустым (иначе nut or {} его подменит)
set -eu
...
PATH = "/opt/menugen/backend/apps/menu/tests/test_mg_604_portions.py"
src = open(PATH).read()
...
new = '''        class BoomDict(dict):
        ...
        class R:
            # MG_604: непустой dict, иначе "nut or {}" подменит BoomDict на {}
