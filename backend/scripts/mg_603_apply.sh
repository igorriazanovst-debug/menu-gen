#!/usr/bin/env bash
# MG-603 apply: unit-тесты для apps/recipes/serializers.py (77% → 100%)
set -eu
...
$COMPOSE exec -T backend python -c "import ast; ast.parse(open('apps/recipes/tests/test_mg_603_serializers.py').read()); print('test_mg_603_serializers.py: AST OK')"

echo
echo "--- pytest test_mg_603_serializers.py ---"
$COMPOSE exec -T backend pytest apps/recipes/tests/test_mg_603_serializers.py -v --tb=short 2>&1 | tail -40
